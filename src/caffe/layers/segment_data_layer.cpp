...
void SegmentDataLayer<Dtype>::GetDistribParam() {
  ...
  else
  {
    ...
    if(param_.coloraug_hsv_h()>0)
    {
      range = ((caffe::caffe_rng_rand()%INT_MAX)*1.0f /(INT_MAX -1) -0.5);
      coloraug_hsv_h_ = range  * param_.coloraug_hsv_h() + 1.0;
    }
  }
}
template <typename Dtype>
...
void SegmentDataLayer<Dtype>::GetImageTop(string src,shared_ptr<Blob<Dtype> > top,int item_id,TopInfo topinfo)
{
  ...
  bool keep_aspect_ratio = topinfo.keep_aspect_ratio();
  if( !keep_aspect_ratio)
  {
    cv::resize(cv_img_origin,cv_img_origin,cv::Size(width,height),0,0,resizetype);  
  }
  if(topinfo.use_color_aug() && cv_img_origin.channels() == 3)
  {
    std::vector<cv::Mat> mv;
    split(cv_img_origin,mv);
    
    mv[0].convertTo(mv[0],CV_64F);
    //LOG(INFO)<<"this->coloraug_hsv_h_ "<<this->coloraug_hsv_h_;
    mv[0] = mv[0]*this->coloraug_hsv_h_;    
    mv[0].convertTo(mv[0],CV_8U);

    //cv::Mat temp;
    //cv::imwrite("1.jpg",cv_img_origin);
    merge(mv,cv_img_origin);
    //cv::imwrite("2.jpg",cv_img_origin);
  }
  ...
  {   cv::Mat_<float> M = Get_Affine_matrix(dstCenter,srcCenter,alpha,scale_local);
      ...
      mAffineWarp(inv_M, cv_img_origin, cv_img, resizetype,topinfo.default_pad_value());
  } 
