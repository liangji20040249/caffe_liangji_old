...
void Solver<Dtype>::Test_Segment(TestProto testproto) {
	...
	int ignore_label = testproto.ignore_label();
        int select_one_label = testproto.select_one_label();
	while(infile >> line)
	{
		...
		cv::resize(img,resizeimg,cv::Size(width,height),0,0,cv::INTER_LINEAR);
		cv::resize(label,resizelabel,cv::Size(outwidth,outheight),0,0,cv::INTER_NEAREST);
		vector<Mat> inputs;
		...
		float acc=0;
		int pred,lb;
		unsigned int valid_eva_count=0;
		for(int h=0;h<resizelabel.rows;h++)
		{
			for(int w=0;w<resizelabel.cols;w++)
			{
				...
				pred = static_cast<int>(maxid);
				lb = static_cast<int>(resizelabel.at<unsigned char>(h,w));

				if(ignore_label>=0 && lb == ignore_label)
					continue;
				if(select_one_label >=0)
				{
					if(lb == select_one_label)
						lb = 1;
					else
						lb = 0;
				}
		...
		if(valid_eva_count == 0)
			...
			acc = acc / static_cast<float>(valid_eva_count);
	...
	for(int k=0;k<total_insec.size();k++)
	{
		...
		if(ignore_label>=0&&ignore_label == k)
			continue;
		else
		{
			mean_iou += iou;
	...
	if(valid_classnum>0)
		mean_iou /= static_cast<float>(valid_classnum);
	
	if(ignore_label>=0)
		LOG(INFO)<<"MEAN IOU (ignore "<<ignore_label<<"):"<<mean_iou;
	else
		LOG(INFO)<<"MEAN IOU:"<<mean_iou;
...
string Solver<Dtype>::SnapshotFilename(const string extension) {
  if(param_.save_onlyone_model())
  {
    return param_.snapshot_prefix() + "_iter_" + "lastest"
    + extension;
  }
  else
  {
