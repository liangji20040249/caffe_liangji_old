...
std::vector<float> forwardNet_getfea(Net<float> &caffe_net,std::string outputlayername, cv::Mat inputimg,int height,int width,float * datamean,float datascale,bool auto_scale=false,bool auto_resize=false)
{ 
  ...
  if(outidx<0)
  {
     LOG(INFO)<<"can not find layer "<<outputlayername;
     return result;
...
int test_extractfea_new() {
  ...
  float loss = 0;
   ...
   LOG(INFO)<<"after load mean data"; 
	...
	std::vector< float > result = forwardNet_getfea(caffe_net, outputlayername, cv_img_origin, height, width,datamean,FLAGS_datascale,FLAGS_autoscale,FLAGS_autoresize);
        //LOG(INFO)<<"after forward";
        timecost = ((double)cv::getTickCount() - timecost)*1000/cv::getTickFrequency();
    	if(result.size()<1)
		{
...
/*
	...
	std::string saveimgname=savefolder + savename +".jpg";

        cv::imwrite(saveimgname, resultimg);
  }
...
RegisterBrewFunction(test_extractfea_new);
